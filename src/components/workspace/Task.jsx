        ...
        {taskArr?.map((task, index) => (
          <div key={index} className="border shadow-lg m-4">
            <div className="mx-4 my-6">
              ...
              <p className="text-[18px] text-green-800">{getCompleted(task.todos)} %</p>
              <p className="underline" onClick={()=>handleOpen(task)}>view in detail</p>
            </div>
            {isOpen && <Modal isOpen={isOpen} onClose={handleClose} task={currTask}/>}
          </div>
        ))}
      </div>
    </section>
